...
import { Home, MessageCircle, Brain, Settings, Sparkles, User } from "lucide-react";
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:top-0 md:bottom-auto">
      <div className="mx-auto max-w-2xl md:max-w-full">
        <div className="flex items-center justify-between md:justify-start gap-0.5 md:gap-1 px-2 md:px-4 py-2 md:py-0 bg-card/80 backdrop-blur-xl border-t md:border-b md:border-t-0 border-border">
          {/* Logo - desktop only */}
          ...
          {links.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `flex flex-col md:flex-row items-center gap-0.5 md:gap-2 px-3 md:px-4 py-2 md:py-3 rounded-lg text-[10px] md:text-sm font-medium transition-colors relative ${
